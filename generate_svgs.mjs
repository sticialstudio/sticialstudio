...
const createPiSVG = (modelLabel, chipLabel) => {
    return wrapSVG(`
      ...
      <!-- RAM Chip (Pi 4/5 have separate RAM) -->
      ...
      <!-- Ports -->
      <!-- USB / Ethernet blocks cluster on right side -->
      ...
      <!-- Styling/Logo -->
      <text x="180" y="290" font-family="sans-serif" font-size="20" font-weight="900" fill="#a7f3d0">Raspberry Pi ${modelLabel}</text>
    `);
};
...
fs.writeFileSync(path.join(outDir, 'board_pi3.svg'), createPiSVG('3', 'BCM2837'));
fs.writeFileSync(path.join(outDir, 'board_pi4.svg'), createPiSVG('4', 'BCM2711'));
fs.writeFileSync(path.join(outDir, 'board_pi5.svg'), createPiSVG('5', 'BCM2712'));
